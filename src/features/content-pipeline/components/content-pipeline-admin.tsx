...
import type { ContentTarget, LessonDraftRecord, StructuredLessonDraft } from "@/features/content-pipeline/types";
...
async function api<T>(url: string, init?: RequestInit): Promise<T> {
  ...
  if (!response.ok || !payload.success || payload.data === undefined) throw new Error(payload.error?.message ?? "Yêu cầu không thành công.");
  return payload.data;
}
...
export function ContentPipelineAdmin() {
  ...
  const refresh = useCallback(async () => {
    try {
      const [targetData, draftData] = await Promise.all([
        api<{ items: ContentTarget[] }>("/api/admin/content-targets"),
        api<{ items: LessonDraftRecord[] }>("/api/admin/lesson-drafts"),
      ]);
  ...
  async function submitSource(event: FormEvent<HTMLFormElement>) {
    ...
    if (!targetLessonId) { setError("Hãy chọn bài học đích."); return; }
    setBusy(true); setError(null); setMessage("Đang tải tài liệu lên vùng lưu trữ riêng tư...");
    try {
      const source = await api<{ id: number }>("/api/admin/content-sources", { method: "POST", body: formData });
      setMessage("Đang trích xuất và chia đoạn nguồn...");
      await api(`/api/admin/content-sources/${source.id}/extract`, { method: "POST" });
      setMessage("9Router đang tạo lesson draft có citation...");
      const generated = await api<{ lessonDraftId: number }>(`/api/admin/content-sources/${source.id}/generate`, {
        method: "POST",
        body: JSON.stringify({ targetLessonId: Number(targetLessonId) }),
      });
      form.reset();
      await refresh();
  ...
  async function openDraft(id: number) {
    ...
    try { setSelected(await api<LessonDraftRecord>(`/api/admin/lesson-drafts/${id}`)); }
    catch (caught: unknown) { setError(caught instanceof Error ? caught.message : "Không thể tải draft."); }
    finally { setBusy(false); }
  }

  function updateSection(index: number, field: "heading" | "bodyMarkdown", value: string) {
    setSelected((current) => current ? { ...current, sections: current.sections.map((section, sectionIndex) => sectionIndex === index ? { ...section, [field]: value } : section) } : current);
  }

  async function saveDraft() {
    if (!selected) return;
    setBusy(true); setError(null);
    const body: StructuredLessonDraft = { title: selected.title, summary: selected.summary, estimatedMinutes: selected.estimatedMinutes, sections: selected.sections };
    try {
      await api(`/api/admin/lesson-drafts/${selected.id}`, { method: "PATCH", body: JSON.stringify(body) });
      await openDraft(selected.id); await refresh(); setMessage("Đã lưu revision mới; draft cần được duyệt lại.");
    } catch (caught: unknown) { setError(caught instanceof Error ? caught.message : "Không thể lưu draft."); setBusy(false); }
  }
  ...
  async function review(decision: "approved" | "rejected" | "needs_revision") {
    ...
    setBusy(true); setError(null);
    try {
      await api(`/api/admin/lesson-drafts/${selected.id}/reviews`, { method: "POST", body: JSON.stringify({ decision }) });
      await openDraft(selected.id); await refresh(); setMessage(`Đã cập nhật: ${decision}.`);
    } catch (caught: unknown) { setError(caught instanceof Error ? caught.message : "Không thể ghi review."); setBusy(false); }
  }
  ...
  async function publish() {
    ...
    setBusy(true); setError(null);
    try {
      const result = await api<{ coursePublished: boolean }>(`/api/admin/lesson-drafts/${selected.id}/publish`, { method: "POST" });
      await openDraft(selected.id); await refresh();
      setMessage(result.coursePublished ? "Đã publish lesson và course ra catalog." : "Đã publish lesson; course vẫn ẩn cho đến khi mọi nội dung hoàn tất.");
    } catch (caught: unknown) { setError(caught instanceof Error ? caught.message : "Không thể publish draft."); setBusy(false); }
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader><CardTitle>1. Tạo draft từ tài liệu</CardTitle><CardDescription>PDF có text layer, DOCX, TXT hoặc Markdown; tối đa 10 MiB.</CardDescription></CardHeader>
        <CardContent>
          <form onSubmit={submitSource} className="grid gap-4 md:grid-cols-[1fr_1fr_auto] md:items-end">
            <div><label htmlFor="source-file" className="mb-1 block text-sm font-medium">Tài liệu nguồn</label><input id="source-file" name="file" type="file" required accept=".pdf,.docx,.txt,.md,text/plain,text/markdown,application/pdf,application/vnd.openxmlformats-officedocument.wordprocessingml.document" className="block w-full rounded-lg border border-slate-300 p-2 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500" /></div>
            <div><label htmlFor="target-lesson" className="mb-1 block text-sm font-medium">Bài học đích</label><select id="target-lesson" value={targetLessonId} onChange={(event) => setTargetLessonId(event.target.value)} required className="h-10 w-full rounded-lg border border-slate-300 bg-white px-3 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500"><option value="">Chọn bài học</option>{targets.map((target) => <option key={target.lessonId} value={target.lessonId}>{target.courseTitle} / {target.chapterTitle} / {target.lessonTitle}</option>)}</select></div>
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(260px,0.7fr)_minmax(0,1.3fr)]">
        <Card><CardHeader><CardTitle>2. Hàng chờ kiểm duyệt</CardTitle><CardDescription>{drafts.length} draft gần nhất</CardDescription></CardHeader><CardContent><ul className="space-y-2">{drafts.map((draft) => <li key={draft.id}><button type="button" onClick={() => void openDraft(draft.id)} className="w-full rounded-xl border border-slate-200 p-3 text-left transition hover:border-indigo-300 hover:bg-indigo-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500"><span className="block font-medium text-slate-900">{draft.title}</span><span className="mt-1 block text-xs text-slate-500">Revision {draft.revision} · {statusLabel[draft.status]}</span></button></li>)}</ul>{!drafts.length ? <p className="text-sm text-slate-500">Chưa có draft.</p> : null}</CardContent></Card>

        <Card><CardHeader><CardTitle>3. Chỉnh sửa, duyệt và publish</CardTitle><CardDescription>Mọi thay đổi tạo revision mới và xóa trạng thái approve cũ.</CardDescription></CardHeader><CardContent>{selected ? <div className="space-y-4">
        ...
        </div> : <p className="text-sm text-slate-500">Chọn một draft trong hàng chờ để kiểm duyệt.</p>}</CardContent></Card>
      </div>
    </div>
  );
}
