...
import { getLessonDraftDetail, updateLessonDraft } from "@/features/content-pipeline/services/content-pipeline-service";
...
export async function PATCH(request: Request, context: RouteContext) {
  try {
    return NextResponse.json(
      { success: true, data: await updateLessonDraft((await context.params).id, await readPipelineJson(request)) },
      { headers: { "Cache-Control": "no-store" } }
    );
  } catch (error) {
    return contentPipelineErrorResponse(error);
