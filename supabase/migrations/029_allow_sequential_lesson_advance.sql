...
begin
  ...
  if v_progress.status = 'locked' then
    select ordered.previous_lesson_id
    into v_previous_lesson_id
    from (
      select
        lesson.id as lesson_id,
        lag(lesson.id) over (
          order by chapter.chapter_order, lesson.lesson_order, lesson.id
        ) as previous_lesson_id
      from public.lessons as lesson
      join public.chapters as chapter on chapter.id = lesson.chapter_id
      where chapter.course_id = v_course_id
        and chapter.is_published = true
        and lesson.is_published = true
    ) as ordered
    where ordered.lesson_id = p_lesson_id;

    if v_previous_lesson_id is null or not exists (
      select 1
      from public.user_progress as previous_progress
      where previous_progress.user_id = v_user_id
        and previous_progress.lesson_id = v_previous_lesson_id
        and previous_progress.status <> 'locked'
    ) then
  ...
  set
    status = case
      when progress.status in ('locked', 'unlocked') then 'in_progress'::public.progress_status
      else progress.status
    end,
    started_at = case
      when progress.status in ('locked', 'unlocked') then coalesce(progress.started_at, now())
